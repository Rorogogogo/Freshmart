...
// Mock categories data - reusing the hierarchical structure from the hierarchy route
const categories = [
  // Root categories
  {
    id: 'cat-1',
...
export async function GET(request: Request) {
  // Get url search params to support filtering
  const { searchParams } = new URL(request.url)
  const includeSubcategories =
    searchParams.get('includeSubcategories') === 'true'
  const searchTerm = searchParams.get('searchTerm')

  // Filter the categories based on search params
  let filteredCategories = [...categories]

  // Filter by parent/child structure if needed
